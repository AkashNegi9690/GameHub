...
export function Geographyquiz() {
    ...
    const fetchgeographyQuestions = async () => {
        try {
            const response = await axios.get(
                "https://opentdb.com/api.php?amount=11&category=22&difficulty=easy&type=multiple"
            );
            setgeographyQuestions(response.data.results);
        } catch (err) {
    ...
    return (
        <>
        <Navbar site="Quizio"/>
            {currentQuestionIndex < geographyQuestions.length - 1 && (
                <div className="max-w-96 mx-auto p-4 bg-white rounded-lg shadow-lg mt-20 ">
                    <h1 className="text-3xl font-bold text-center mb-4">Geography Quiz</h1>
                    <div className="mb-4">
