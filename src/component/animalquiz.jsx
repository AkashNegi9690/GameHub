...
export function AnimalsQuiz() {
    ...
    useEffect(() => {
        if (animalsQuestions.length > 0) {
            const currentQuestion = animalsQuestions[currentQuestionIndex];
            const options = [...currentQuestion.incorrect_answers, currentQuestion.correct_answer];
            const shuffled = shuffleOptions(options);
            setShuffledOptions(shuffled); // Set shuffled options in state
        }
    }, [currentQuestionIndex, animalsQuestions]);
    ...
    return (
        <>
        <Navbar site="Quizio"/>
            {currentQuestionIndex < animalsQuestions.length - 1 && (
                <div className="max-w-96 mx-auto p-4 bg-white rounded-lg shadow-lg mt-20 ">
                    <h1 className="text-3xl font-bold text-center mb-4">Animals Quiz</h1>
                    <div className="mb-4">
